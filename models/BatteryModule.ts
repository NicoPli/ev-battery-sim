...
export class BatteryModule {
  ...
  updateTemperature(current: number, deltaTimeHours: number, coolingPower: number): void {
    ...
    const effectiveResistance = 0.001 * 15; // 15x multiplier for more noticeable temperature effects
    const heatGenerated = Math.pow(current, 2) * effectiveResistance * deltaTimeHours * 0.7;
    ...
    const passiveCooling = tempDifferential * 0.2 * deltaTimeHours;
    
    // Active cooling (from the cooling system, scales with cooling power setting)
    const activeCooling = tempDifferential * 0.5 * coolingPower * deltaTimeHours;
